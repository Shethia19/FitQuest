...
class CalendarAgendaState extends State<CalendarAgenda>
  ...
  widget.weekDay == WeekDay.long
      ? DateFormat('EEEE', _locale).format(date)
      : DateFormat('E', _locale).format(date),
  style: TextStyle(
    fontSize: widget.dayNameFontSize ?? 12.0,
    color: isSelected ? widget.selectedDateColor : widget.dateColor,
...
class DateFormat {
  ...
  // Static named constructors for common formats (optional)
  static DateFormat yMMMM([String? locale]) => DateFormat('yMMMM', locale);
  static DateFormat E([String? locale]) => DateFormat('E', locale);
  static DateFormat EEEE([String? locale]) => DateFormat('EEEE', locale);

  String format(DateTime date) {
    // Mocked output for testing
    return '$pattern formatted date for locale: $locale';
  }
}
